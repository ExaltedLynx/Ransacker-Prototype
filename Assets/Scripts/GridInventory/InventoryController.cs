...
public class InventoryController : MonoBehaviour
{
    ...
    /*
    //TODO only use items fully within hotbar space
    //loops over the bottom two rows of the player inventory, which acts as the hotbar
    public void UseHotbarItems()
    {
        int gridHeight = playerInventory.currentGridHeight;
        for (int x = 0; x < playerInventory.currentGridWidth; x++)
        {
            for (int y = gridHeight - 2; y < gridHeight; y++)
            {
                InventoryItem hotbarItem = playerInventory.GetItem(x, y);
                ...
                if(hotbarItem != null)
                {
                    switch (hotbarItem.itemData)
                    {
                        case ConsumableItem cItem:
                            bool wasUsed = cItem.OnUse();
                            if(wasUsed)
                                playerInventory.RemoveAndDestroyItem(x, y);
                            break;
                        ...
                        case WeaponItem:
                            ...
                            {
                                playerInventory.RemoveItem(x, y);
